...
class SilverCouponWidget extends StatelessWidget {
  const SilverCouponWidget(
...
class CouponViewV2 extends StatelessWidget {
  ...
  final ValueChanged<CouponModel> onTap;
  final SilverBuyViewModel silverBuymodel;

  @override
  Widget build(BuildContext context) {
    final locale = locator<S>();
    return silverBuymodel.focusCoupon != null
        ? IndividualCouponView(
            appliedCode: silverBuymodel.appliedCoupon?.code,
            model: silverBuymodel.focusCoupon!,
            desc: silverBuymodel.focusCoupon!.description!,
            silverBuymodel: silverBuymodel,
            disabledDesc: silverBuymodel.focusCoupon!.disabledDescription ?? '',
            ticketMultiplier: silverBuymodel.focusCoupon!.ticketMultiplier ?? 0,
            icon: silverBuymodel.focusCoupon!.icon,
            isDisabled: silverBuymodel.focusCoupon!.couponSubType != null &&
                silverBuymodel.focusCoupon!.couponSubType == 'SUPER_FELLO' &&
                locator<UserService>().baseUser!.superFelloLevel !=
                    SuperFelloLevel.SUPER_FELLO,
            onTap: (coupon) => silverBuymodel.applyCoupon(
              coupon.code,
              false,
            ),
          )
        : GestureDetector(
            onTap: () {
              silverBuymodel.buyFieldNode.unfocus();
              silverBuymodel.showOfferModal(silverBuymodel);
            },
            child: Container(
...
class SilverCouponPage extends StatelessWidget {
  SilverCouponPage({required this.model, super.key});
  final SilverBuyViewModel? model;
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider.value(
      ...
      child: Scaffold(
        ...
        body: Consumer<SilverBuyViewModel>(
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        ...
        if (silverBuymodel.appliedCoupon == null ||
            silverBuymodel.appliedCoupon?.code != model.code) {
          if (!silverBuymodel.couponApplyInProgress) onTap(model);
        }
      },
      child: Container(
        padding: EdgeInsets.symmetric(
              ...
              children: [
                AppImage(
                  ...
                  color: model.minPurchase! <=
                              int.parse(
                                silverBuymodel.silverAmountController!.text,
                              ) &&
                          !isDisabled
                      ? UiConstants.teal2
                ...
                const Spacer(),
                silverBuymodel.couponApplyInProgress &&
                        silverBuymodel.couponCode == model.code
