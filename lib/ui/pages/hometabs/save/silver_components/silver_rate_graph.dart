...
class LineSilverGradientChart extends StatefulWidget {
  const LineSilverGradientChart({super.key});

  @override
  State<LineSilverGradientChart> createState() => _LineGradientChartState();
}

class _LineGradientChartState extends State<LineSilverGradientChart> {
  Map? responseData;
  List<String>? returnsList;
  List<SilverChartData>? silverChartData;
  List<SilverChartData>? _chartData;

  List<SilverChartData>? get chartData => _chartData;
...
set selectedIndex(int value) {
  ...
  Future.delayed(const Duration(milliseconds: 100)).then((_) {
    int startIndex = silverChartData!.length - (12 * (selectedIndex + 1));
    if (startIndex < 0) startIndex = 0;

    chartData = silverChartData!.sublist(startIndex, silverChartData!.length);
  });
  ...
  @override
  void initState() {
    getSilverGraphChartData();
    super.initState();
  }

  Future<void> getSilverGraphChartData() async {
    final res = await locator<GetterRepository>().getSilverRatesGraphItems();
    if (res.isSuccess()) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
        ...
        silverChartData = responseData!["chartDataList"];
        selectedIndex = 4;
      });
    }
  }
                ...
                SizedBox(
                  ...
                  child: SfCartesianChart(
                    ...
                    series: [
                      ...
                      SplineSeries(
                        color: const Color.fromARGB(255, 167, 186, 202),
                        width: 1,
                        enableTooltip: true,
                        animationDelay: 0,
                        animationDuration: 0,
