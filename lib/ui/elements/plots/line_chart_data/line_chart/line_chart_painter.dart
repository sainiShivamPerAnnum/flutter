...
import 'package:felloapp/ui/elements/plots/line_chart_data/axis_chart/axis_chart_painter.dart';
import 'package:felloapp/ui/elements/plots/line_chart_data/base_chart/base_chart_painter.dart';
import 'package:felloapp/ui/elements/plots/line_chart_data/utils/canvas_wrapper.dart';
import 'package:felloapp/ui/elements/plots/line_chart_data/extensions/canvas_extension.dart';
import 'package:felloapp/ui/elements/plots/line_chart_data/utils/utils.dart';
import 'package:flutter/material.dart';
import 'package:felloapp/ui/elements/plots/line_chart_data/extensions/paint_extension.dart';
...
class LineChartPainter extends AxisChartPainter<LineChartData> {
  ...
  Path _generateBarPath(Size viewSize, LineChartBarData barData,
    ...
    if (barData.isStepLineChart) {
      return _generateStepBarPath(viewSize, barData, barSpots, holder,
  ...
  Path _generateNormalBarPath(Size viewSize, LineChartBarData barData,
      List<FlSpot> barSpots, PaintHolder<LineChartData> holder,
      {Path appendToPath}) {
  ...
  Path _generateStepBarPath(Size viewSize, LineChartBarData barData,
      List<FlSpot> barSpots, PaintHolder<LineChartData> holder,
      {Path appendToPath}) {
    viewSize = getChartUsableDrawSize(viewSize, holder);
    final path = appendToPath ?? Path();
    ...
    if (topTitles.showTitles) {
      ...
      while (horizontalSeek <= data.maxX) {
        if (topTitles.checkToShowTitle(
          ...
          final span = TextSpan(
              style: topTitles.getTextStyles(horizontalSeek), text: text);
          final tp = TextPainter(
              text: span,
              textAlign: TextAlign.center,
    ...
    if (bottomTitles.showTitles) {
      ...
      while (horizontalSeek <= data.maxX) {
        if (bottomTitles.checkToShowTitle(data.minX, data.maxX, bottomTitles,
          ...
          final span = TextSpan(
              style: bottomTitles.getTextStyles(horizontalSeek), text: text);
          final tp = TextPainter(
              text: span,
              textAlign: TextAlign.center,
